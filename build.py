...
PLANS = [("Месяц","199 ₽","в месяц",[ "1 ключ v2raytun","Все платформы","Полная скорость"],False),
         ("6 месяцев","149 ₽","в месяц · −25%",["Выгоднее на 25%","Все платформы","Приоритетные серверы"],True),
         ("Год","125 ₽","в месяц · −37%",["Максимальная выгода","Все платформы","Приоритетная поддержка"],False)]
def tariffs():
    c=""
    for name,price,sub,feats,hit in PLANS:
        fl="".join(f'<li>{esc(f)}</li>' for f in feats)
        c+=f'<div class="vk-plan{" vk-plan-hit" if hit else ""}">{"<span class=vk-plan-badge>Хит</span>" if hit else ""}<h3>{esc(name)}</h3><div class="vk-price">{esc(price)}</div><div class="vk-price-sub">{esc(sub)}</div><ul>{fl}</ul><a class="vk-btn vk-btn-block" href="{LK}">Купить ключ</a></div>'
    import json as _j
    ld={"@context":"https://schema.org","@type":"Product","name":"Ключ v2raytun","description":"Ключи и подписка v2raytun",
        "offers":{"@type":"AggregateOffer","priceCurrency":"RUB","lowPrice":"125","highPrice":"199","offerCount":"3"}}
    return f'<section class="vk-sec" id="tarify"><div class="vk-wrap"><h2 class="vk-h2">Тарифы на ключи v2raytun</h2><p class="vk-sub">Пробный ключ — бесплатно. Подписка — от 125 ₽/мес.</p><div class="vk-plans">{c}</div></div></section><script type="application/ld+json">{_j.dumps(ld,ensure_ascii=False)}</script>'

def page(slug, title, desc, h1, body, cluster=None, faqs=None):
    path = "/" if slug == "" else f"/{slug}/"
    parts = [head(title, desc, path), header(), '<main>']
    if slug == "":
        parts += [hero(), stat(), bento(), platforms(), body, tariffs()]
...
PAGES.append(dict(slug="", title="Ключи v2raytun — скачать и получить конфиг | v2ray2tun",
    desc="Рабочие ключи v2raytun для ПК, Android, iPhone, Mac и TV. Бесплатный пробный ключ, конфигурации VLESS Reality, подписка от 125 ₽.",
    h1="", cluster="Общее",
...
PAGES.append(dict(slug="klyuchi", title="Ключи v2raytun — бесплатные ключи и конфиги подключения",
    ...
    faqs=[("Чем ключ отличается от подписки?","Ключ — это один сервер. Подписка — ссылка, которая автоматически обновляет список ключей и серверов."),
          ("Сколько действует бесплатный ключ?","Пробного ключа хватает на ознакомление; для постоянного использования подойдёт подписка от 125 ₽/мес.")]))
# Скачать
PAGES.append(dict(slug="skachat", title="Скачать v2raytun — официальное приложение и ключ",
    ...
    h1="Скачать v2raytun", cluster="Скачать",
...
PAGES.append(dict(slug="na-pk", title="v2raytun на ПК и Windows — скачать и ключ",
    ...
    h1="v2raytun на ПК и Windows", cluster="ПК/Windows",
...
PAGES.append(dict(slug="android", title="v2raytun на Android — скачать APK и получить ключ",
    ...
    h1="v2raytun на Android (APK)", cluster="Android",
...
PAGES.append(dict(slug="ios", title="v2raytun на iPhone и iOS — ключ и настройка",
    ...
    h1="v2raytun на iPhone (iOS)", cluster="iOS",
...
PAGES.append(dict(slug="mac", title="v2raytun на Mac — скачать и добавить ключ",
    ...
    h1="v2raytun на Mac", cluster="Mac",
...
PAGES.append(dict(slug="tv", title="v2raytun на Smart TV и Android TV — ключ",
    ...
    h1="v2raytun на Smart TV", cluster="TV",
...
PAGES.append(dict(slug="podpiska", title="Подписка v2raytun — ключи с автообновлением",
    desc="Подписка v2raytun: одна ссылка с автообновлением серверов и ключей. Тарифы от 125 ₽/мес, пробный доступ бесплатно.",
    ...
    body=prose("Подписка v2raytun — это ссылка, которую добавляют в приложение один раз. Она сама обновляет список серверов и ключей, поэтому подключение всегда рабочее.")+
        ...
        h2("Подписка v2raytun бесплатно")+prose("Попробовать можно бесплатно — пробный доступ. Полная подписка без ограничений скорости стоит от 125 ₽/мес."),
...
PAGES.append(dict(slug="github", title="v2raytun GitHub — официальные сборки и ключ",
    ...
    h1="v2raytun GitHub", cluster="Бренд/офиц/github",
...
PAGES.append(dict(slug="tarify", title="Тарифы v2raytun — цены на ключи и подписку",
    desc="Тарифы на ключи v2raytun: пробный ключ бесплатно, подписка от 125 ₽/мес. Месяц, 6 месяцев и год.",
    ...
    body=prose("Пробный ключ v2raytun — бесплатно. Для постоянного использования выбери подписку: чем дольше период, тем дешевле месяц.")+
        '<div class="vk-plans">'+"".join(
            (lambda name,price,sub,feats,hit: f'<div class="vk-plan{" vk-plan-hit" if hit else ""}">{"<span class=vk-plan-badge>Хит</span>" if hit else ""}<h3>{esc(name)}</h3><div class="vk-price">{esc(price)}</div><div class="vk-price-sub">{esc(sub)}</div><ul>{"".join(f"<li>{esc(f)}</li>" for f in feats)}</ul><a class="vk-btn vk-btn-block" href="{LK}">Купить ключ</a></div>')(*p)
            for p in PLANS)+'</div>',
...
ARTICLES = [
 ...
 dict(slug="besplatnyj-klyuch-ili-podpiska", date="2026-05-27",
   ...
   body=prose("Перед оплатой логично попробовать бесплатный ключ v2raytun, а затем решить, нужна ли подписка. Сравним варианты.")+
        ...
        h2("Что выбрать")+prose("Для разового использования хватит бесплатного ключа. Для постоянной работы выгоднее подписка — от 125 ₽/мес при оплате за год.")),
]
...
/* tariffs */
.vk-plans{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:24px}
.vk-plan{background:var(--panel);border:1px solid var(--line);border-radius:var(--rad);padding:26px 22px;position:relative;display:flex;flex-direction:column}
.vk-plan-hit{border-color:var(--amb);box-shadow:0 0 0 1px var(--amb),0 18px 40px rgba(124,58,237,.16)}
.vk-plan-badge{position:absolute;top:-11px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,var(--amb),var(--amb2));color:#1a1205;font-size:12px;font-weight:700;padding:4px 14px;border-radius:999px}
.vk-price{font-size:34px;font-weight:800;margin:10px 0 2px}.vk-price-sub{color:var(--mut);font-size:13px;margin-bottom:16px}
...
@media(max-width:900px){
 ...
 .vk-bento,.vk-plans{grid-template-columns:1fr}
...
@media(max-width:560px){.vk-plat{grid-template-columns:repeat(2,1fr)}.vk-fbot{flex-direction:column;gap:6px}.vk-fgrid{grid-template-columns:1fr;gap:22px}}
